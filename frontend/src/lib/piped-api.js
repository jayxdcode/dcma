...
function normalizeVideoItem(item) {
  return {
    id: extractVideoId(item?.url || item?.videoId || item?.watchUrl || item?.id || item?.video_id || item?.vid),
    title: item?.title || item?.name || item?.titleText || item?.caption || '',
    url: item?.url || item?.watchUrl || (item?.videoId ? `/watch?v=${item.videoId}` : ''),
    cover: item?.thumbnail || item?.thumbnailUrl || (item?.thumbnails && item.thumbnails[0]) || null,
...
export async function search(q, opts = {}) {
  ...
  for (const c of candidates) {
    try { pushCandidate(c); } catch (e) { /* ignore */ }
  }
